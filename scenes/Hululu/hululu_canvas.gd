...
func play_dream():
	...
	(
		...
		. parallel()
		. tween_property(
			shader_mat, "shader_parameter/wave_intensity", wave_intensity, transition_duration
		)
		. set_ease(ease_mode)
	...
	(
		...
		. parallel()
		. tween_property(
			shader_mat, "shader_parameter/noise_strength", noise_strength, transition_duration
		)
		. set_ease(ease_mode)
		. set_trans(Tween.TRANS_SINE)
	)
	...
	(
		tween
		. parallel()
		. tween_property(shader_mat, "shader_parameter/wave_intensity", 0.0, transition_duration)
		. set_ease(ease_mode)
		. set_trans(Tween.TRANS_SINE)
	)
	(
		tween
		. parallel()
		. tween_property(shader_mat, "shader_parameter/noise_strength", 0.0, transition_duration)
		. set_ease(ease_mode)
		. set_trans(Tween.TRANS_SINE)
	)
